...
for (const section of sections) {
  ...
  const meta = section.match(
    /<!-- tag: (\S+) \| target: ([0-9a-f]{40}) \| prerelease: (true|false) -->/
  )
...
for (const e of entries) {
  ...
  if (result.ok) {
    if (e.status === "MISSING") {
      ...
      const dates = DRY_RUN
        ? ""
        : spawnSync("gh", ["api", `repos/${REPO}/releases/tags/${e.tag}`, "-q", ".created_at"], {
            encoding: "utf-8"
          }).stdout?.trim() ?? ""
      console.log(
        `✓ created ${e.tag}${dates ? ` (created_at: ${dates}, expected ~${e.date})` : ""}`
      )
    } else {
      updated++
